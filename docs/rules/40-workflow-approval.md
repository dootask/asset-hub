...
- 字段（见 `web/lib/db/schema.ts`）：
  - `requires_approval`：布尔值，指明该操作是否必须审批。
  - `default_approver_type`：`none | user | role`。
  - `default_approver_refs`：JSON 数组，存储默认审批人/角色 ID。
  - `allow_override`：是否允许申请人修改审批人。
  - `metadata`：JSON 预留扩展（例如额外提示、自动化策略）。
- 维护方式：
  - 代码端通过 `DEFAULT_ACTION_CONFIGS` 提供基线配置，迁移脚本会在首次运行时写入；
  - 系统页面 `/system/approval` 允许管理员在 UI 上调整上述字段，并通过 `PUT /api/config/approvals/:id` 持久化。
...
- 每次创建审批请求时都应把与之对应的 `asset_action_configs` 结果拷贝到 `metadata.configSnapshot`，以便后续的 `applyApprovalAction`/审计使用，即使管理员之后修改了配置也不影响历史记录。
...
- 客户端侧通过 `@dootask/tools` 获取宿主上下文：`appReady`、`isMicroApp`、`selectUsers`、`fetchUserBasic`、`getThemeName` 等。
- 服务器端通过 `web/lib/integrations/dootask-notifications.ts` 封装的网关向宿主推送审批通知；若未配置宿主 API，则降级为日志输出。

设计约定：

- 审批表单必须根据 `isMicroApp` 结果决定交互：在宿主里打开 `selectUsers` 组件，独立模式则展示只读默认审批人或允许手输。
- `/api/approvals` 创建/更新时只调用 `notifyApprovalCreated` / `notifyApprovalUpdated` 之类的统一函数，避免在 Route Handler 内直接依赖具体宿主实现，便于在本地/独立部署时降级。
