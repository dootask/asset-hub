...
import { listDeletedConsumables } from "@/lib/repositories/consumables";
import {
  CONSUMABLE_STATUSES,
...
function parseStatuses(searchParams: URLSearchParams): ConsumableStatus[] | undefined {
  const rawStatuses = [
  ...
  const normalized = Array.from(new Set(rawStatuses)).filter((value) =>
    CONSUMABLE_STATUSES.includes(value as ConsumableStatus),
  ) as ConsumableStatus[];
...
export async function GET(request: Request) {
  ...
  const result = listDeletedConsumables({
