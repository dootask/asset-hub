...
export default function ConsumableDeleteRestoreActions({
  consumableId,
  locale,
  isDeleted,
  onActionComplete,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await client.delete(`/apps/asset-hub/api/consumables/${consumableId}`, {
      ...
      onActionComplete?.();
      if (redirectOnDelete) {
        router.push(`/${resolvedLocale}/consumables/list`);
      }
      router.refresh();
      feedback.success(isChinese ? "耗材已删除。" : "Consumable deleted.");
    } catch (error) {
      const message = extractApiErrorMessage(
        error,
        isChinese ? "删除失败，请稍后重试。" : "Failed to delete consumable.",
      );
  ...
  const handleRestore = async () => {
    ...
    try {
      const client = await getApiClient();
      await client.post(`/apps/asset-hub/api/consumables/${consumableId}/restore`);
      setRestoreOpen(false);
      onActionComplete?.();
      router.refresh();
      feedback.success(isChinese ? "耗材已恢复。" : "Consumable restored.");
    } catch (error) {
      const message = extractApiErrorMessage(
        error,
        isChinese ? "恢复失败，请稍后重试。" : "Failed to restore consumable.",
      );
      feedback.error(message, {
        blocking: true,
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        <AlertDialog open={restoreOpen} onOpenChange={setRestoreOpen}>
          <AlertDialogTrigger asChild>
            <Button variant="outline" className="rounded-full">
              {isChinese ? "恢复耗材" : "Restore"}
            </Button>
          </AlertDialogTrigger>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
                {isChinese ? "确认恢复耗材" : "Restore consumable?"}
              </AlertDialogTitle>
              <AlertDialogDescription>
                {isChinese
                  ? "恢复后该耗材将重新出现在列表中。"
                  : "The consumable will be visible again after restoring."}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        <AlertDialog open={permanentOpen} onOpenChange={setPermanentOpen}>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
                {isChinese ? "永久删除耗材" : "Delete consumable forever?"}
              </AlertDialogTitle>
              <AlertDialogDescription>
                {isChinese
              ...
              <AlertDialogAction
                ...
                onClick={async () => {
                  ...
                  try {
                    const client = await getApiClient();
                    await client.delete(
                      `/apps/asset-hub/api/consumables/${consumableId}/permanent`,
                    );
  ...
  return (
    <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
      <AlertDialogTrigger asChild>
        <Button variant="destructive" className="rounded-full">
          {isChinese ? "删除耗材" : "Delete"}
        </Button>
      </AlertDialogTrigger>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            {isChinese ? "确认删除耗材" : "Delete consumable?"}
          </AlertDialogTitle>
          <AlertDialogDescription>
            {isChinese
              ? "删除后可在后台恢复，请填写删除原因。"
