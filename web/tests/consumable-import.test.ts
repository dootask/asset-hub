...
import { parseConsumableImportRows } from "@/app/api/consumables/import/route";
...
describe("Consumable import parser", () => {
  it("parses valid rows", () => {
    const rows = [
      ["name", "category", "companyCode", "quantity", "unit", "keeper", "location", "safetyStock", "description"],
      ["硒鼓", "PrinterSupplies", "HITOSEA", 10, "pcs", "Admin", "Shanghai", 5, "示例"],
    ];
    const result = parseConsumableImportRows(rows);
  ...
  it("derives status from quantity and safetyStock", () => {
    const rows = [
      ["Name", "Category", "Company_Code", "Quantity", "Unit", "Keeper", "Location", "Safety_Stock", "Description"],
  ...
  it("respects archived status input when provided", () => {
    const rows = [
      ["name", "category", "status", "companyCode", "quantity", "unit", "keeper", "location", "safetyStock"],
